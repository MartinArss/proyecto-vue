<template>
  ...
  <div class="w3-top w3-hide-large w3-hide-medium" id="myNavbar">
    <div class="w3-bar w3-black w3-opacity w3-hover-opacity-off w3-center w3-small">
      ...
      <a href="/listarpuesto" class="w3-bar-item w3-button" style="width:25% !important">Listado puesto</a>
    ...
    <div class="w3-padding-large" id="main">
      ...
      <div class="card">
        <div class="card-header" style="font-weight: bold; color: black;">Usuarios</div>
  
        <div class="card_body">
          <button class="button button1" style="margin-left: 55%; width: 15%; position: fixed;"><RouterLink to="/crearusuario" class="btn btn-outline-success" style="width: 100%;">Crear</RouterLink></button>
          <button class="button button1" style="margin-left: 82%; width: 15%;">
            <router-link :to="{ path: '/editarusuario' }" class="btn btn-outline-info" style="width: 100%;">Editar</router-link>
          </button>
          <table class="table">
            <thead>
              <tr>
                <th style="font-weight: bold; font-size: 24px;">Identificador</th>
                <th style="font-weight: bold; font-size: 24px;">Nombre de usuario</th>
                <th style="font-weight: bold; font-size: 24px;">Contraseña</th>
                <!-- <th style="font-weight: bold; font-size: 24px;">Fecha de registro</th> -->
                <th style="font-weight: bold; font-size: 24px;">Nombre de empleado</th>
                <th style="font-weight: bold; font-size: 24px;">Rol establecido</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="user in usuario" :key="user.pkUsuario">
                <td style="font-weight: bolder; font-size: 24px; color: #FF7474;">{{ user.pkUsuario }}</td>
                <td style="font-weight: bolder; font-size: 16px; color: #534873;">{{ user.user }}</td>
                <td style="font-weight: bolder; font-size: 16px; color: #534873;">{{ user.password }}</td>
                <!-- <td style="font-weight: bolder; font-size: 16px; color: #534873;">{{ user.fechaRegistro }}</td> -->
                <td style="font-weight: bolder; font-size: 16px; color: #534873;">{{ user.empleado.nombre + ' ' + user.empleado.apellido}}</td>
                <td style="font-weight: bolder; font-size: 16px; color: #534873;">{{ user.rol.nombre }}</td>
                <td>
                  <div class="btn-group" role="label" aria-label="">
                    <button type="button" v-on:click="borrarUsuario(user.pkUsuario)"  class="btn btn-danger">Eliminar</button>  
                  </div>
                </td>
              </tr>
            </tbody>
  ...
  export default {
    ...
    methods: {
      consultarUsuario() {
        axios.get("https://localhost:7289/Usuario").then((result) => {
      ...
      borrarUsuario(id) {
          ...
          if(pregunta===true){
            axios.delete("https://localhost:7289/Usuario?id=" + id);
