<template>
  ...
  <a href="/listarpuesto" class="w3-bar-item w3-button w3-padding-large w3-hover-black" style="background-color: #000; color: #fff; font-style: italic;">
    <p style="font-weight: bolder;">Listado puestos</p>
  </a>
  ...
  <div class="w3-top w3-hide-large w3-hide-medium" id="myNavbar">
    ...
    <div class="w3-padding-large" id="main">
      ...
      <div class="card">
        <div class="card-header" style="font-weight: bold; color: black;">Puestos</div>
  
        <div class="card_body">
          <button class="button button1" style="margin-left: 55%; width: 15%; position: fixed;"><RouterLink to="/crearpuesto" class="btn btn-outline-success" style="width: 100%;">Crear</RouterLink></button>
          <button class="button button1" style="margin-left: 82%; width: 15%;">
            <router-link :to="{ path: '/editarpuesto' }" class="btn btn-outline-info" style="width: 100%;">Editar</router-link>
          </button>
          <table class="table">
            <thead>
              <tr>
                <th style="font-weight: bold; font-size: 24px;">Identificador</th>
                <th style="font-weight: bold; font-size: 24px;">Nombre del puesto</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="puestos in puesto" :key="puestos.pkPuesto">
                <td style="font-weight: bolder; font-size: 24px; color: #FF7474;">{{ puestos.pkPuesto }}</td>
                <td style="font-weight: bolder; font-size: 16px; color: #534873;">{{ puestos.nombre }}</td>
                <td>
                  <div class="btn-group" role="label" aria-label="">
                    <button type="button" v-on:click="borrarPuesto(puestos.pkPuesto)" class="btn btn-danger">Eliminar</button>  
                  </div>
                </td>
              </tr>
            </tbody>
  ...
  export default {
    ...
    methods: {
      ...
      borrarPuesto(id) {
          var pregunta=window.confirm('Desea eliminar el puesto?');
    
          if(pregunta===true){
            axios.delete("https://localhost:7289/Puesto?id=" + id);
