<template>
  ...
  <div class="w3-top w3-hide-large w3-hide-medium" id="myNavbar">
    ...
    <div class="w3-padding-large" id="main">
      ...
      <div class="card">
        <div class="card-header" style="font-weight: bold; color: black;">Departamentos</div>
  
        <div class="card_body">
          <button class="button button1" style="margin-left: 55%; width: 15%; position: fixed;"><RouterLink to="/creardepartamento" class="btn btn-outline-success" style="width: 100%;">Crear</RouterLink></button>
          <button class="button button1" style="margin-left: 82%; width: 15%;">
            <router-link :to="{ path: '/editardepartamento' }" class="btn btn-outline-info" style="width: 100%;">Editar</router-link>
          </button>
          <table class="table">
            <thead>
              <tr>
                <th style="font-weight: bold; font-size: 24px;">Identificador</th>
                <th style="font-weight: bold; font-size: 24px;">Nombre del departamento</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="depa in departamento" :key="depa.pkDepartamento">
                <td style="font-weight: bolder; font-size: 24px; color: #FF7474;">{{ depa.pkDepartamento }}</td>
                <td style="font-weight: bolder; font-size: 16px; color: #534873;">{{ depa.nombre }}</td>
                <td>
                  <div class="btn-group" role="label" aria-label="">
                    <button type="button" v-on:click="borrarDepartamento(depa.pkDepartamento)" class="btn btn-danger">Eliminar</button>  
                  </div>
                </td>
              </tr>
            </tbody>
  ...
  export default {
    ...
    methods: {
      ...
      borrarDepartamento(id) {
          var pregunta=window.confirm('Desea Eliminar el departamento?');
    
          if(pregunta===true){
            axios.delete("https://localhost:7289/Departamento?id=" + id);
