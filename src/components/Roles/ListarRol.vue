<template>
  ...
  <a href="/listarrol" class="w3-bar-item w3-button w3-padding-large w3-hover-black" style="background-color: #000; color: #fff; font-style: italic;">
    <p style="font-weight: bolder;">Listado rols</p>
  </a>
  ...
  <div class="w3-top w3-hide-large w3-hide-medium" id="myNavbar">
    ...
    <div class="w3-padding-large" id="main">
      ...
      <div class="card">
        ...
        <div class="card_body">
          <button class="button button1" style="margin-left: 55%; width: 15%; position: fixed;"><RouterLink to="/crearrol" class="btn btn-outline-success" style="width: 100%;">Crear</RouterLink></button>
          <button class="button button1" style="margin-left: 82%; width: 15%;">
            <router-link :to="{ path: '/editarrol' }" class="btn btn-outline-info" style="width: 100%;">Editar</router-link>
          </button>
          <table class="table">
            <thead>
              <tr>
            ...
            <tbody>
              <tr v-for="rols in rol" :key="rols.pkRol">
                <td style="font-weight: bolder; font-size: 24px; color: #FF7474;">{{ rols.pkRol }}</td>
                <td style="font-weight: bolder; font-size: 16px; color: #534873;">{{ rols.nombre }}</td>
                <td>
                  <div class="btn-group" role="label" aria-label="">
                    <button type="button" v-on:click="borrarPuesto(rols.pkRol)" class="btn btn-danger">Eliminar</button>  
                  </div>
                </td>
              </tr>
            </tbody>
  ...
  export default {
    ...
    methods: {
      ...
      borrarRol(id) {
          var pregunta=window.confirm('Desea eliminar el rol?');
    
          if(pregunta===true){
            axios.delete("https://localhost:7289/Rol?id=" + id);
